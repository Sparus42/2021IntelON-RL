...
class CityFlow_Jinan_3x4(gym.Env):
    """
    Description:
    ...
    def __init__(self):
        ...
        self.config_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "Jinan_3x4_config")
        ...
        self.action_space = spaces.Box(0, 9, shape=[12,0], dtype=np.int_)
        print(len(np.shape(self.action_space)))
        if self.mode == "all_all":
            self.observation_space = spaces.Box(0, np.inf, shape=[self.num_lanes,0], dtype=np.int_)
        else:
            #dummy
            self.observation_space = spaces.Box(0, np.inf, shape=[1,0], dtype=np.int_)
